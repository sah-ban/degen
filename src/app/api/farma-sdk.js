...
class FarmaSDK {
  constructor(config) {
    ...
    if (fullKeyBytes.length < 64) {
      throw new Error(
        "Invalid private key length. Expected at least 64 bytes (32 private + 32 public)."
      );
    }
  ...
  async request(method, path, body = null) {
    ...
    options.headers["X-Public-Key"] = `${
      this.config.isAdmin ? "0" : this.config.frameId
    }:${this.publicKey64}`;
    options.headers["X-Date"] = date;

    console.log(
      `Making ${method} request to https://${options.hostname}:${options.port}${options.path}`
    );
