...
export default function Demo(
  ...
  const [activeDiv, setActiveDiv] = useState<'Home' | 'AllowanceTable' | 'TipsTable' | 'Leaderboard'>('Home'); 
...
const [tipsReceivedData, setTipsReceivedData] = useState<TipsReceivedResponse | null>(null);
const fetchTipsReceived = useCallback(async (fid: string) => {
  try {
    ...
    if (!tipsReceivedResponse.ok) {
      throw new Error(`Fid HTTP error! Status: ${tipsReceivedResponse.status}`);
    }
...
{pointsData?.pointsRank && rainData?.rainPoints ? (
 ...
 <div className="flex-grow"></div>
 {activeDiv==='Home'&&  <div className="flex-none pr-4">
    <Wallets wallet_addresses={allowanceData?.data[0]?.wallet_addresses?? []} />
  </div>}
</header>
  ...
  function LeaderBoard( ) {
    ...
    return (
          ...
          <div className="w-auto bg-slate-900 flex flex-col h-[calc(100vh-130px)]" {...swipeHandlers}>
      {/* Header */}
      <header>
...
key={index} className="odd:bg-slate-700 even:bg-slate-600">
       ...
       <td className="py-2 cursor-pointer" onClick={()=>sdk.actions.viewProfile({ fid: Number(item?.recipient_fid) })}>@{item?.recipient_username ?? "N/A"}</td>
       <td className="px-4 py-2">{item?.tip_amount ?? "N/A"}</td>
       <td className={item?.tip_status === "invalid" ? "text-red-500" : ""}>{item?.tip_status ?? "N/A"}</td>
       <td className="px-4 py-2 text-3xl cursor-pointer" onClick={()=>sdk.actions.viewCast({hash:(item?.cast_hash).replace(/\\/, '0')})}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
 ...
 <tr className="border-2 border-lime-400">
  ...
  <div className="flex items-center cursor-pointer" onClick={()=>sdk.actions.viewProfile({ fid: Number(profileData?.fids) })}>
     @{profileData?.username ?? "N/A"}
       ...
       <td className="py-2">{allowanceData?.data[0]?.tip_allowance ?? "N/A"}</td>
       <td className="py-2">{allowanceData?.data[0]?.remaining_tip_allowance ?? "N/A"}</td>
  </tr>
...
key={index}
  ...
  <div className="flex items-center cursor-pointer" onClick={()=>sdk.actions.viewProfile({ fid: Number(item?.fid) })}>
    @{item?.username ?? "N/A"}
       ...
       <td className="py-2">{item?.remaining_tip_allowance ?? "N/A"}</td>
     </tr>
   ))}
</tbody>
  </table>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6">
  ...
  >
    ...
    `}</style>
      ...
      {!castHash ? "Share the mini-app to claim few $DEGEN" : ( isConfirming ? "Claiming...": ( isConfirmed ? "Claimed": "Thanks for sharing"   )  )}
  </button>
      ...
      {Array.isArray(allowanceData?.data) && allowanceData?.data.length > 0 && (
            <a href="https://degensub.vercel.app"
...
>
   ...
   function Blocked( ) {
    return (
      ...
      <div className="text-center p-8 m-8 bg-white shadow-lg rounded-xl">
        ...
        <p className="text-gray-600 mt-4">You do not have permission to use this mini app.</p>
        <button 
  ...
  <table className="table-auto w-full bg-slate-700 text-lime-400 text-center">
    ...
    {Array.isArray(tipsReceivedData?.tipsReceivedData) && tipsReceivedData?.tipsReceivedData.length > 0 ? (
 <tbody>
...
key={index} className="odd:bg-slate-700 even:bg-slate-600">
       ...
       <td className="py-2 cursor-pointer" onClick={()=>sdk.actions.viewProfile({ fid: Number(item?.fid) })}>@{item?.username ?? "N/A"}</td>
       <td className="px-4 py-2">{item?.tip_amount ?? "N/A"}</td>
       <td className={item?.tip_status === "invalid" ? "text-red-500" : ""}>{item?.tip_status ?? "N/A"}</td>
       <td className="px-4 py-2 text-3xl cursor-pointer" onClick={()=>sdk.actions.viewCast({hash:(item?.cast_hash).replace(/\\/, '0')})}><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
      ...
      <tbody>
<tr>
<td className="px-4 py-4" colSpan={5}>No Tips Received</td>
      </tr>
      </tbody>
      ...
      <header>
        <div className="container mx-auto px-4 text-center text-white">
          <div className="flex justify-around">
            <button
              className={`p-2 ${
                activeTip === 'Received' ? 'border-b-4 border-sky-400 text-sky-400 font-bold' : ''
              }`}
            ...
            <button
              className={`p-2 ${
                activeTip === 'Sent' ? 'border-b-4 border-sky-400 text-sky-400 font-bold' : ''
              }`}
